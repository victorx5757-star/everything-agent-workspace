...
export function BatchActionToolbar() {
  ...
  const handleBatchUpdate = async (updates: Partial<UpdateIssueRequest>) => {
    try {
      ...
      toast.success(t('issues:batch.updated', { count }));
    } catch {
      toast.error(t('issues:batch.failedToUpdate'));
    }
  };
  ...
  const handleBatchDelete = async () => {
    try {
      ...
      clear();
      toast.success(t('issues:batch.deleted', { count }));
    } catch {
      toast.error(t('issues:batch.failedToDelete'));
    } finally {
      setDeleteOpen(false);
    }
  };
    ...
    <>
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-1 rounded-lg border bg-background px-2 py-1.5 shadow-lg">
        <div className="flex items-center gap-1.5 pl-1 pr-2 border-r mr-1">
          <span className="text-sm font-medium">{t('issues:batch.selected', { count })}</span>
          <button
            type="button"
            onClick={clear}
        ...
        <StatusPicker
          ...
          trigger={t('issues:fields.status')}
          align="center"
        />
        ...
        <PriorityPicker
          ...
          triggerRender={<Button variant="ghost" size="sm" disabled={loading} />}
          trigger={t('issues:fields.priority')}
          align="center"
        />
        ...
        <AssigneePicker
          ...
          triggerRender={<Button variant="ghost" size="sm" disabled={loading} />}
          trigger={t('issues:fields.assignee')}
          align="center"
        />
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t('issues:batch.deleteConfirm', { count })}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {t('issues:batch.deleteDescription', { count })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
