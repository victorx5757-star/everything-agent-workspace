...
export function TokensTab() {
  ...
  const loadTokens = useCallback(async () => {
    try {
      ...
      toast.error(e instanceof Error ? e.message : t('tokens.failedToLoad'));
    } finally {
  ...
  const handleCreateToken = async () => {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : t('tokens.failedToCreate'));
    } finally {
  ...
  const handleRevokeToken = async (id: string) => {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : t('tokens.failedToRevoke'));
    } finally {
  ...
  return (
    <div className="space-y-8">
          ...
          <CardContent className="space-y-3">
            ...
            <div className="grid gap-3 sm:grid-cols-[1fr_120px_auto]">
              ...
              <Select value={tokenExpiry} onValueChange={(v) => { if (v) setTokenExpiry(v); }}>
                ...
                <SelectContent>
                  <SelectItem value="30">{t('tokens.days30')}</SelectItem>
                  <SelectItem value="90">{t('tokens.days90')}</SelectItem>
                  <SelectItem value="365">{t('tokens.oneYear')}</SelectItem>
                ...
                <CardContent className="flex items-center gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {tk.token_prefix}... · {t('tokens.created')} {new Date(tk.created_at).toLocaleDateString()} · {tk.last_used_at ? `${t('tokens.lastUsed')} ${new Date(tk.last_used_at).toLocaleDateString()}` : t('tokens.neverUsed')}
                      {tk.expires_at && ` · ${t('tokens.expires')} ${new Date(tk.expires_at).toLocaleDateString()}`}
                    </div>
                  </div>
                  <Tooltip>
                    <TooltipTrigger
                      render={
                        <Button
                          ...
                          disabled={tokenRevoking === tk.id}
                          aria-label={`${t('tokens.revokeTooltip')} ${tk.name}`}
                        >
                          <Trash2 className="h-3.5 w-3.5" />
                        </Button>
                      }
                    />
                    <TooltipContent>{t('tokens.revokeTooltip')}</TooltipContent>
                  </Tooltip>
                </CardContent>
              </Card>
            ))}
      ...
      <AlertDialog open={!!revokeConfirmId} onOpenChange={(v) => { if (!v) setRevokeConfirmId(null); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('tokens.revokeToken')}</AlertDialogTitle>
            <AlertDialogDescription>
        ...
        <DialogContent>
          ...
          <div className="flex items-center gap-2">
            ...
            <Tooltip>
              ...
              />
              <TooltipContent>{t('tokens.copyToken')}</TooltipContent>
            </Tooltip>
          </div>
          <DialogFooter>
